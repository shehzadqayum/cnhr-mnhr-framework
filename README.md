...
🔗 **Live dashboard:** [CNHR-MNHR Dashboard v2.3](https://shehzadqayum.github.io/cnhr-mnhr-framework/dashboard/dashboard_v2_3.html)
...
| Companion Guide | Full explanation of all terms, concepts, strategy, and control system (Feynman-technique examples) | [PDF](operational/Work_Schedule_Companion_Guide.pdf) · [DOCX](operational/Work_Schedule_Companion_Guide.docx) |
...
| Document | Purpose | Link |
|----------|---------|------|
| cnhr_calculator.py | Reference implementation — single source of truth for all numerical values | [Source](tools/cnhr_calculator.py) |
| build_companion_guide.js | Node.js builder for the Companion Guide DOCX | [Source](tools/build_companion_guide.js) |
...
│   ├── Work_Schedule_Companion_Guide.pdf
│   ├── Work_Schedule_Companion_Guide.docx
│   └── build_schedule.py
│
├── dashboard/
│   └── dashboard_v2_3.html
...
| Get a plain-language explanation of every term and concept | [Companion Guide](operational/Work_Schedule_Companion_Guide.pdf) |
| Compute exact numerical values for any configuration | [cnhr_calculator.py](tools/cnhr_calculator.py) |
...
## Three Structural Properties → Control System

Properties implicit in the mathematics, formally derived in [ARCH-001 §3.3](architecture/ARCH-001_Architecture_and_Trajectory.md). Together they form a closed-loop control system ([ARCH-001 §3.4](architecture/ARCH-001_Architecture_and_Trajectory.md)):

**1. NLW Regulatory Attractor (Target).** At minimum δ, CNHR(e) = NLW exactly, regardless of ρ. Higher productivity → more free time, not higher income. The framework is a subsistence optimiser.

**2. Day 3 Checkpoint Optimality (Sensor).** The cost-recovery drag C/(k·s) at s=30h decays to £14.93/hr — the crossover point where the ρ signal (±£4/hr) becomes detectable. Day 3 is also the last moment to enact the sacrifice rule. Optimal sensor placement.

**3. Structural Hours Dominance (Actuator).** ∂CNHR/∂δ is 14× larger than ∂CNHR/∂ρ at d=12. Schedule adjustment (δ) is the dominant control lever — not trip-rate optimisation.

| Element | Framework Equivalent | Property |
|---------|---------------------|----------|
| Target | NLW = £12.21/hr | Property 1 — attractor |
| Sensor | CNHR at s=30h (Day 3) | Property 2 — crossover |
| Actuator | δ adjustment (sacrifice day) | Property 3 — 14× leverage |
| Feedback | Day-3 checkpoint + sacrifice rule | All three combined |
...
### Planned
- Tax layer (Income Tax + Class 4 NI as third cost category T scaling with profit; 26% marginal rate)
